    ...
    \item \textbf{Valutazione dell'errore}:
        \begin{itemize}
            \item Una volta costruito il polinomio, si valuta l'errore
                $\left\lvert p(x)-f(x) \right\rvert$ per ogni $x\in[a,b]$ per
...
interpolante nella forma di Newton usa una particolare base chiamata base
di Newton, che viene costruita da punti 

La base di Newton è definita dalle seguenti $n+1$ funzioni base linearmente
indipendenti 

Il polinomio interpolante nella base di Newton è una combinazione lineare di
queste funzioni base. Lo si può scrivere come:
$$p(x)=b_0+b_1(x-x_0)+b_2(x-x_0)(x-x_1)+\ldots+b_n(x-x_0)(x-x_1)\ldots(x-x_n)$$

Imponendo le condizioni di interpolazione a questo polinomio si ottiene:
\begin{equation*}
    \begin{aligned}
        p(x_0)=y_0\Rightarrow
        b_0+b_1\overbrace{(x_0-x_0)}^0+b_2\overbrace{(x_0-x_0)}^0(x_0-x_1)+\ldots+b_n\overbrace{(x_0-x_0)}^0(x_0-x_1)\ldots(x_0-x_{n-1})&=y_0\\ 
        & b_0=y_0                                                                                                                            
    \end{aligned} 
\end{equation*}
